...
local main = lib:NewTab("Main", "in game only, not while in lobby - Made by connanz1234")
local lobby = lib:NewTab("Lobby", "functions to get the current gifts from event - Made by connanz1234")
...
main:NewDropdown("Grab/Get (Drop down)",{"Buttons","Items drops"},"hi",function(selected)
    ...
    if selected == "Buttons" then
        ...
        task.spawn(getInstance, "ButtonPrompt")
    end
    if selected == "Items drops" then
        
        task.spawn(getInstance, "GrabPrompt")
    end
end)
